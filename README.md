...
4. **模型训练 (Step 6)**：采用 **ST-GCN** 架构。
   ...
   - **时序层**：
## 📂 4. 输入构建流程说明
1. **核心逻辑**：从“路段”到“路径”的转换。

    逻辑实现：代码通过 pd.date_range 创建了以 1 分钟（1min）为间隔的频率槽位。

    索引映射：将每一个绝对时间戳映射到 0 到 13 的整数索引中。这确保了不同车辆在同一分钟内的行为能够被准确地累加到同一个时间切片（Time Slice）中。 
    (row['timestamp'] - start_time).total_seconds() // 60 
2. **节点定义**：路径视为节点。

    路径编码：通过 df_all['edge_id'].apply(tuple) 将原本的路段列表转化为可哈希的元组，作为路径的唯一标识。

    频率筛选：由于 14 分钟内产生的路径可能上千，采用 value_counts().head(50) 筛选出流量最大的 50 条路径作为模型训练的“节点”（Nodes）。
